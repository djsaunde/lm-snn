...
def get_2d_input_weights():
    ...
    weight_matrix = np.zeros((n_e_sqrt, n_e_sqrt))
    rearranged_weights = np.zeros((n_e_sqrt, n_e_sqrt))
    connMatrix = connections[name][:]
    weight_matrix = np.array([entry for entry in connMatrix]).reshape((n_e_sqrt, n_e_sqrt))
...
for name in population_names:
    ...
    for conn_type in recurrent_conn_names:
        ...
        connections[connName].connect(neuron_groups[connName[0:2]], neuron_groups[connName[2:4]], weightMatrix * np.random.random(weightMatrix.shape[0]))
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        if connName[0] == 'X' and connName[2] == 'A':
            connections[connName].connect_one_to_one(input_groups[connName[0:2]], neuron_groups[connName[2:4]], delay=delay[connType])
        else:
            connections[connName].connect(input_groups[connName[0:2]], neuron_groups[connName[2:4]], weightMatrix * np.random.random(weightMatrix.shape[0]), delay=delay[connType])
