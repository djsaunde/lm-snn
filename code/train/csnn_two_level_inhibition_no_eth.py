...
model_name = 'csnn_two_level_inhibition_no_eth'
						...
						if feature != other_feature:
							...
							for n in xrange(n_e):
								connections['AiAe'][feature * n_e + n, other_feature * n_e + n] = \
										min(max_inhib, end_low_inhib * np.sqrt(euclidean([x, y], [x_, y_])))
...
if __name__ == '__main__':
	...
	parser.add_argument('--start_low_inhib', type=float, default=0.5, help='The beginning value \
														of inhibiton for the increasing scheme.')
	parser.add_argument('--end_low_inhib', type=float, default=15.0, help='The middle value \
														of inhibiton for the increasing scheme.')
	...
	ending = '_'.join([ str(conv_size), str(conv_stride), str(conv_features), str(n_e), \
									...
									str(start_low_inhib), str(end_low_inhib), str(max_inhib) ])
