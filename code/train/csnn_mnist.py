...
def save_connections():
    ...
    print '...saving connections: ' + weights_dir + save_conns[0] + '_' + stdp_input
    ...
    for conn_name in save_conns:
        ...
        # save it out to disk
        np.save(data_path + weights_dir + conn_name + '_' + stdp_input, conn_list_sparse)
...
def save_theta():
    ...
    for pop_name in population_names:
        ...
        print '...saving theta: ' + weights_dir + 'theta_' + pop_name + '_' + ending + '_' + stdp_input

        # save out the theta parameters to file
        np.save(data_path + weights_dir + 'theta_' + pop_name + '_' + ending + '_' + stdp_input, neuron_groups[pop_name + 'e'].theta)
...
if test_mode:
    weight_path = data_path + weights_dir
...
while j < num_examples:
    ...
    # otherwise, record results and confinue simulation
        ...
        if j % update_interval == 0 and j > 0:
            ...
            target = open(performance_dir + weights_name + '_' + stdp_input + '.txt', 'w')
            target.truncate()
...
if not test_mode:
    ...
    np.save(data_path + activity_dir + 'resultPopVecs' + str(num_examples) + '_' + stdp_input + '_' + ending, result_monitor)
    np.save(data_path + activity_dir + 'inputNumbers' + str(num_examples) + '_' + stdp_input + '_' + ending, input_numbers)
