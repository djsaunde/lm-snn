...
model_name = 'csnn_growing_inhibition_negative_weights'
...
def normalize_weights():
	...
	'''
	connection = input_connections['XeAe'][:].todense()
	for feature in xrange(conv_features):
		...
		for n in xrange(n_e):
			dense_weights = input_connections['XeAe'][:, feature * n_e + n].todense()
			dense_weights[convolution_locations[n]] *= column_factors[n]
			input_connections['XeAe'][:, feature * n_e + n] = dense_weights
...
def plot_weights_and_assignments(assignments):
	...
	image3 = ax3.matshow(reshaped_assignments, cmap=color, vmin=-1.5, vmax=9.5)
	ax3.set_title('Neuron labels')
	...
	plt.colorbar(image2, cax=cax2)
	plt.colorbar(image3, cax=cax3, ticks=np.arange(-1, 10))
	...
	fig.canvas.draw()
	return fig, ax1, ax2, ax3, image1, image2, image3


def update_weights_and_assignments(fig, ax1, ax2, ax3, im1, im2, im3, assignments, spike_counts):
	'''
	Update the plot of the weights from input to excitatory layer to view during training.
	'''
	weights = get_2d_input_weights()
	im1.set_array(weights)
	im2.set_array(-(1 - weights))
	
	reshaped_assignments = assignments.reshape((int(np.sqrt(n_e_total)), int(np.sqrt(n_e_total)))).T
	im3.set_array(reshaped_assignments)
	...
	for x in xrange(features_sqrt):
		for y in xrange(features_sqrt):
			...
			if c > 0:
				ax3.text(x, y, str(c), va='center', ha='center', weight='heavy', fontsize=16)
			else:
				ax3.text(x, y, '', va='center', ha='center')
...
def run_train():
	...
	if do_plot:
		...
		weights_assignments_figure, weights_axes, neg_weights_axes, \
						assignments_axes, weights_image, neg_weights_image, \
	...
	input_connections['YeAe'].W = 0.0 * input_connections['XeAe'].W
	...
	while j < num_examples:
		...
		if j > 250:
			input_connections['YeAe'].W = -(1 - input_connections['XeAe'].W)
		...
		if j % weight_update_interval == 0 and do_plot:
			update_weights_and_assignments(weights_assignments_figure, weights_axes, neg_weights_axes, \
													assignments_axes, weights_image, neg_weights_image, \
													assignments_image, assignments, current_spike_count)
...
if __name__ == '__main__':
	...
	ending = '_'.join([ str(conv_size), str(conv_stride), str(conv_features), str(n_e), \
						...
						str(proportion_grow), str(noise_const), str(neg_multiplier) ])
