...
model_name = 'conv_two_level_mnist'
...
def normalize_weights():
	...
	for conn_name in input_connections:
		...
		for n in xrange(n_neurons):
			fltr = connection[:, n]
			sum_ = np.sum(np.asarray(fltr), axis=0)
			factors = weight['ee_input'] / sum_

			dense_weights = input_connections[conn_name][:, n].todense()
			dense_weights *= factors
...
def predict_label(assignments, spike_rates, accumulated_rates, spike_proportions):
	...
	for scheme in voting_schemes:
		...
		if scheme == 'all':
				...
				if num_assignments[i] > 0:
					for idx in np.where(assignments == i)[0]:
						if np.sum(spike_rates[idx]) != 0: 
							neighbors = [idx]
							neighbors.extend(get_neighbors(idx, n_neurons_sqrt))
			...
			neighborhood_activity = np.zeros([n_neurons, n_e])
			most_spiked_array = np.array(np.zeros((n_neurons, n_e)), dtype=bool)
			...
			for n in xrange(n_neurons):
				...
				column_sums = np.sum(spike_rates[n : n + 1, :], axis=0)

				# find the excitatory neuron which spiked the most
				most_spiked_array[n, np.argmax(column_sums)] = True
...
def build_network():
	...
	for name in input_connection_names:
		...
		for conn_type in input_conn_names:
			...
			input_connections[conn_name].connect(input_groups[conn_name[0:2]], neuron_groups[conn_name[2:4]],
																	weight_matrix, delay=delay[conn_type])
			...
			if j == increase_iter:
				for n in xrange(n_neurons):
					for other_n in xrange(n_neurons):
						if n != other_n:
							for n in xrange(n_e):
								connections['AiAe'][n * n_e + n, \
									other_n * n_e + n] = max_inhib
...
if __name__ == '__main__':
	...
	parser.add_argument('--start_inhib', type=float, default=2.5)
	parser.add_argument('--max_inhib', type=float, default=17.5)
