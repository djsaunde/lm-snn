...
model_name = 'csnn_growing_inhibition_double_training'
...
def run_activity_plotting():
	...
	ending = '_'.join([ str(conv_size), str(conv_stride), str(conv_features), str(n_e), \
						...
						str(proportion_grow), str(noise_const), str(different_seed), str(inhibition_level) ])
...
if __name__ == '__main__':
	...
	parser.add_argument('--different_seed', type=str, default='True', help='Whether to use a different \
												random seed on the second pass through the training data.')
	...
	for var in [ 'do_plot', 'plot_all_deltas', 'reset_state_vars', 'test_max_inhibition', \
					...
					'save_spikes', 'weights_noise', 'test_adaptive_threshold', 'interactive_mode', 'different_seed' ]:
		if locals()[var] == 'True':
			locals()[var] = True
	...
	ending = '_'.join([ str(conv_size), str(conv_stride), str(conv_features), str(n_e), \
						str(num_train), str(random_seed), str(normalize_inputs), 
						str(proportion_grow), str(noise_const), str(different_seed) ])
