...
model_name = 'csnn_growing_inhibition_gaussian_weights'
...
def build_network():
	...
	if record_spikes and do_plot:
		...
		b.ion()
		b.subplot(211)
		b.raster_plot(spike_monitors['Ae'], refresh=1000 * b.ms, \
			showlast=1000 * b.ms, title='Excitatory spikes per neuron')
		b.subplot(212)
		b.raster_plot(spike_monitors['Ai'], refresh=1000 * b.ms, \
			showlast=1000 * b.ms, title='Inhibitory spikes per neuron')
		b.tight_layout()
		fig_num += 1
	...
	for name in input_connection_names:
		...
		for conn_type in input_conn_names:
			...
			if test_mode:
				...
				elif weights_type == 'unimodal':
					for feature in xrange(conv_features):
						mode_x, mode_y = np.random.randint(0, high=28), np.random.randint(0, high=28)
						normal_distro = scipy.stats.multivariate_normal(mean=[mode_x, mode_y], \
												cov=[[gauss_variance, 0], [0, gauss_variance]])
						for n in xrange(n_e):
							for idx in xrange(conv_size ** 2):
								idx_x, idx_y = idx // 28, idx % 28
								input_connections[conn_name][convolution_locations[n][idx], \
												feature * n_e + n] = normal_distro.pdf([idx_x, idx_y])
				

				elif weights_type == 'multimodal':
					for feature in xrange(conv_features):
						normal_distros = []
						for mode in xrange(n_modes):
							mode_x, mode_y = np.random.randint(0, high=28), np.random.randint(0, high=28)
							normal_distros.append(scipy.stats.multivariate_normal(mean=[mode_x, mode_y], \
															cov=[[gauss_variance, 0], [0, gauss_variance]]))
						for n in xrange(n_e):
							for idx in xrange(conv_size ** 2):
								idx_x, idx_y = idx // 28, idx % 28
								input_connections[conn_name][convolution_locations[n][idx], \
										feature * n_e + n] = np.sum([normal_distro.pdf([idx_x, \
													idx_y]) for normal_distro in normal_distros ])
...
if __name__ == '__main__':
	...
	parser.add_argument('--weights_type', type=str, default='unimodal', help='Type of synapse weight \
									initialization to use. One of "uniform", "unimodal", "multimodal".')
	parser.add_argument('--n_modes', type=int, default=3, help='The number of modes to consider when \
