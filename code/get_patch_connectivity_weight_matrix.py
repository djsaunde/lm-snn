...
print '\n'
print '\n'.join([ str(idx + 1) + ' | ' + file_name for idx, file_name in enumerate([ file_name for file_name in sorted(os.listdir(weight_path)) if 'AeAe' in file_name and 'all' in file_name ]) ])
print '\n'

to_get = raw_input('Enter the index of the file from above which you\'d like to plot: ')

file_name = [ file_name for file_name in sorted(os.listdir(weight_path)) if 'AeAe' in file_name and 'all' in file_name ][int(to_get) - 1]
