...
echo "csnn_growing_inhibition_small_nu_ee_pre_job.sh"

python csnn_growing_inhibition_small_nu_ee_pre.py --mode=train --num_train=$num_train --conv_size=$conv_size --conv_stride=$conv_stride --noise_const=$noise_const \
	--conv_features=$conv_features --random_seed=$random_seed --normalize_inputs=$normalize_inputs --proportion_grow=$proportion_grow

python csnn_growing_inhibition_small_nu_ee_pre.py --mode=test --num_test=10000 --conv_size=$conv_size --conv_stride=$conv_stride --proportion_grow=$proportion_grow \
