...
plt.title('ETH vs. LM-SNN: 70% accuracy sample requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/70_percent_samples_needed.png')
...
plt.title('ETH vs. LM-SNN: 70% accuracy FLOPs requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/70_percent_flops_needed.png')
...
kmeans = [np.argmax(curve > 70) * 250 for curve in kmeans_curves.values()]
nns = [np.argmax(curve > 70) * 250 for curve in nn_curves.values()]
...
plt.legend(); plt.grid()
plt.savefig('../../plots/70_percent_samples_needed_nn_kmeans.png')
...
plt.grid(); plt.legend()
plt.savefig('../../plots/70_percent_flops_needed_nn_kmeans.png')
plt.show()
