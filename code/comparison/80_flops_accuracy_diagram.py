...
sizes = [225, 400, 625]
eth = [10500, 12500, 18500]
lmsnn = [7000, 7500, 6500]
...
plt.title('ETH vs. LM-SNN: 80% accuracy sample requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/80_percent_samples_needed.png')
...
plt.title('ETH vs. LM-SNN: 80% accuracy FLOPs requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/80_percent_flops_needed.png')
...
kmeans = [np.argmax(curve > 80) * 250 for curve in kmeans_curves.values()]
nns = [np.argmax(curve > 80) * 250 for curve in nn_curves.values()]
...
plt.legend(); plt.grid()
plt.savefig('../../plots/80_percent_samples_needed_nn_kmeans.png')
...
plt.grid(); plt.legend()
plt.savefig('../../plots/80_percent_flops_needed_nn_kmeans.png')
plt.show()
