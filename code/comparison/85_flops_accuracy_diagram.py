...
sizes = [225, 400, 625]
eth = [60000, 36500, 35000]
lmsnn = [9000, 8750, 8500]
...
plt.title('ETH vs. LM-SNN: 85% accuracy sample requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/85_percent_samples_needed.png')
...
plt.title('ETH vs. LM-SNN: 85% accuracy FLOPs requirements')

plt.legend(); plt.grid()
plt.savefig('../../plots/85_percent_flops_needed.png')
...
kmeans = [np.argmax(curve > 85) * 250 for curve in kmeans_curves.values()]
nns = [np.argmax(curve > 85) * 250 for curve in nn_curves.values()]
...
plt.legend(); plt.grid()
plt.savefig('../../plots/85_percent_samples_needed_nn_kmeans.png')
...
plt.grid(); plt.legend()
plt.savefig('../../plots/85_percent_flops_needed_nn_kmeans.png')
plt.show()
