...
MNIST_data_path = os.getenv('HOME') + '/data/MNIST/'
...
#                 w_post[i, connection.rowj[i]] = connection.rowdata[i]
            w_post = np.copy(connection)
                
#             rowSums = np.sum(w_post, axis = 1)
# #             colDataEntries = [len(connection.coldataindices[j]) for j in xrange(n_e)]
# #             colMeans = colSums/colDataEntries
#             rowFactors = factor/rowSums
#              
# #             print 'conn sum before', np.sum(w_post, axis = 0), w_post.shape
#             for i in xrange(n_input):#
#                 connection[i,:] *= rowFactors[i]
# #             connection = connections[connName][:]
# #             w_post = np.copy(connection)
# #             print 'conn sum', np.sum(w_post, axis = 0), w_post.shape
...
if use_weight_dependence:
    ...
    weight_normalization_interval = 1
record_states = False
do_plot_performance = True
...
weight['ee_input'] = 78. # 25.0 * 2
weight['ee'] = 5.0 # 
delay['ee_input'] = (3*b.ms,20*b.ms)
...
tc_pre_ie = 20*b.ms
tc_post_ie = 20*b.ms
nu_ee_pre =  0.000001      # learning rate
nu_ee_post = 0.05       # learning rate
nu_ie =      0.001      # learning rate
...
#         eqs_stdp_post_ee = 'w += nu_ee_post * (pre * euler**(exp_ee_post*(w)) - STDP_offset * euler**(exp_ee_pre*(wmax_ee - w))); post += 1.'
        ...
        eqs_stdp_pre_ee = 'pre = 1.'
        eqs_stdp_post_ee = 'post2before = post2; w += nu_ee_post * pre * post2before; post2 = 1.'
...
for name in population_names:
    ...
    if test_mode or weight_path[-8:] == 'weights/':
        ...
        neuron_groups['e'].theta = np.ones((n_e)) * 10.0*b.mV
...
#         weightMatrix = scipy.sparse.lil_matrix(weightMatrix)
        if delay[connType] == (0*b.ms,0*b.ms):
            connections[connName] = b.Connection(input_groups[connName[0:2]], neuron_groups[connName[2:4]], structure= conn_structure, 
                                                    state = 'g'+connType[0])
            connections[connName].connect(input_groups[connName[0:2]], neuron_groups[connName[2:4]], weightMatrix)
        else:
            connections[connName] = b.Connection(input_groups[connName[0:2]], neuron_groups[connName[2:4]], structure= conn_structure, 
...
#                 spike_monitors[name] = SpikeMonitor(neuron_groups[name])
    ...
    if (j%weight_normalization_interval == 0) and not test_mode and not use_weight_dependence:
        print weight_path[-8:]
        normalize_weights()
